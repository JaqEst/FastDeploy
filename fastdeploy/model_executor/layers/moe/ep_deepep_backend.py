...
class DeepEPBufferManager:
    ...
    def clear_buffer(cls):
        if cls._engine:
            cls._engine.clear_ep_buffer()

    @classmethod
    def recreate_buffer(cls):
        if cls._engine:
            cls._engine.create_ep_buffer()
...
class DeepEPRunner(EPRunnerBase):
    """DeepEP-based EP runner base."""
    ...
    def clear_ep_buffer(self):
        self.ep_engine.clear_ep_buffer()

    def create_ep_buffer(self):
        self.ep_engine.create_ep_buffer()


class DeepEPPrefillRunner(DeepEPRunner):
    """
    DeepEP prefill runner (high-throughput mode).
    """
    ...
    def set_allocate_on_comm_stream(allocate_on_comm_stream: bool = False):
        if DeepEPPrefillRunner.allocate_on_comm_stream == allocate_on_comm_stream:
            return
        logger.info(
        ...
        DeepEPPrefillRunner.allocate_on_comm_stream = allocate_on_comm_stream
    ...
    def dispatch(
        ...
        **kwargs,
        ...
        (
            ...
            topk_idx,
            self.num_experts + self.redundant_experts_num,
...
class DeepEPDecoderRunner(DeepEPRunner):
    """DeepEP decoder runner (low-latency mode)."""
